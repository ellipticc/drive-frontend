...
const getCommentTool = (
  ...
  tool<Record<string, never>, void>({
    ...
    execute: async () => {
      ...
      for await (const partialArray of experimental_partialOutputStream) {
        for (let i = lastLength; i < partialArray.length; i++) {
          const comment = partialArray[i];
          if (!comment) continue;
          let parsedComment;
          try {
            parsedComment = commentSchema.parse(comment);
          } catch {
            continue;
          }
    ...
    execute: async () => {
      ...
      for await (const partialArray of experimental_partialOutputStream) {
        for (let i = lastLength; i < partialArray.length; i++) {
          ...
          let parsedCellUpdate;
          try {
            parsedCellUpdate = cellUpdateSchema.parse(cellUpdate);
          } catch {
            continue;
          }
