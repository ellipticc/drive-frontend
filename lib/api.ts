...
class ApiClient {
  ...
  // IndexedDB AI Preferences
  async getIndexedDbPreferences(): Promise<ApiResponse<{ settings: string; hash: string; signature: string; updated_at: string }>> {
    return this.request('/indexeddb/preferences');
  }

  async storeIndexedDbPreferences(payload: { settings: string; hash: string; signature: string; deviceId: string }): Promise<ApiResponse> {
    return this.request('/indexeddb/sync', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  }
