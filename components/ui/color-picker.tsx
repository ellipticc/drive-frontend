...
function hexToRgb(hex: string, alpha?: number): ColorValue {
  ...
  if (result) {
    return {
      ...
      a: alpha ?? (result[4] ? Number.parseInt(result[4], 16) / 255 : 1),
    };
  }
  ...
  if (shorthandResult) {
    return {
      r: Number.parseInt((shorthandResult[1] ?? "0") + (shorthandResult[1] ?? "0"), 16),
      g: Number.parseInt((shorthandResult[2] ?? "0") + (shorthandResult[2] ?? "0"), 16),
      b: Number.parseInt((shorthandResult[3] ?? "0") + (shorthandResult[3] ?? "0"), 16),
      a: alpha ?? (shorthandResult[4] ? Number.parseInt((shorthandResult[4] ?? "f") + (shorthandResult[4] ?? "f"), 16) / 255 : 1),
    };
  }
...
function rgbToHex(color: ColorValue): string {
  ...
  const alpha = color.a < 1 ? toHex(color.a * 255) : "";
  return `#${toHex(color.r)}${toHex(color.g)}${toHex(color.b)}${alpha}`;
}

function rgbToHsv(color: ColorValue, currentHue?: number): HSVColorValue {
  const r = color.r / 255;
  const g = color.g / 255;
  const b = color.b / 255;
...
function parseColorString(value: string): ColorValue | null {
  ...
  if (trimmed.startsWith("#")) {
    const hexMatch = trimmed.match(/^#([a-fA-F0-9]{3}|[a-fA-F0-9]{4}|[a-fA-F0-9]{6}|[a-fA-F0-9]{8})$/);
    if (hexMatch) {
...
const inputGroupItemVariants = cva(
  "h-8 [-moz-appearance:textfield] font-mono focus-visible:z-10 focus-visible:ring-1 [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none",
  {
    variants: {
      position: {
