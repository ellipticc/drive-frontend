...
function useSidebar() {
  ...
  if (!context) {
    // Return a safe fallback when used outside of SidebarProvider
    // This prevents errors on public pages where sidebar isn't needed
    return {
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // This sets the localStorage to keep the sidebar state with error handling.
      try {
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden transition-transform duration-300 ease-in-out data-[state=open]:duration-400 data-[state=closed]:duration-300"
          style={
            {
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-auto overflow-x-hidden pr-6 group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
...
function SidebarMenu({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn("flex w-full min-w-0 flex-col gap-1 list-none pr-3", className)}
      {...props}
    />
  )
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-s px-2.5 py-0.5 list-none",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
