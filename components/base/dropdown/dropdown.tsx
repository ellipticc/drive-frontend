    ...
    return (
        ...
        >
            {(state) => (
                <div
                    className={cx(
                        "relative flex items-center rounded-md px-2.5 py-2 outline-ring transition duration-100 ease-linear",
                ...
                >
                    {Icon && (
                        <Icon
                            aria-hidden="true"
                            className={cx("mr-2 size-4 shrink-0 stroke-[2.25px]", state.isDisabled ? "text-muted-foreground" : "text-muted-foreground")}
                        />
                    )}
                    ...
                    <span
                        className={cx(
                            ...
                            state.isDisabled ? "text-muted-foreground" : "text-foreground",
                    ...
                    {addon && (
                        <span
                            className={cx(
                                "ml-3 shrink-0 rounded px-1 py-px text-xs font-medium ring-1 ring-border",
                                state.isDisabled ? "text-muted-foreground" : "text-muted-foreground",
                            )}
                        >
                            {addon}
                        </span>
...
const DropdownPopover = (props: AriaPopoverProps) => {
    return (
        <AriaPopover
            ...
            className={(state) =>
                cx(
                    "w-62 origin-(--trigger-anchor-point) overflow-auto rounded-lg bg-popover shadow-lg ring-1 ring-border will-change-transform",
                    state.isEntering &&
