...
export const ReasoningTrigger = memo(
  ({
    ...
    return (
      <CollapsibleTrigger
        className={cn(
          "flex w-full items-center gap-2 text-neutral-300 dark:text-stone-500 text-sm transition-colors hover:text-neutral-400 dark:hover:text-stone-400",
          className
        )}
        {...props}
      >
  ...
  ({ className, children, isStreaming, ...props }: ReasoningContentProps) => {
    return (
      <CollapsibleContent
        ...
        {...props}
      >
        <div className="relative pl-9 text-neutral-300 dark:text-stone-500 font-inter text-[13px] leading-relaxed">
          <div className="absolute left-[7px] top-1 bottom-0 w-[1.5px] bg-foreground/20 dark:bg-foreground/30 transition-colors" />
          <MarkdownRenderer
            content={children}
